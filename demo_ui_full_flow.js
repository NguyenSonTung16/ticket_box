...
(async () => {
  ...
  console.log('🎯 Đang cuộn tìm và chọn chung 1 ghế (A-13)...');
  const clickSeat = async (page) => {
    ...
    await page.waitForTimeout(1500); // Khán giả nhìn quá trình cuộn đến ghế
    await seatLocator.dispatchEvent('click');
  };
  ...
  // Khán giả nhìn 2 giây
  await page1.waitForTimeout(2000);

  console.log('⚡ Cùng ấn Tiếp Tục!');
  const BOOK_BUTTON_SELECTOR = 'button:has-text("Tiếp tục")';
  await Promise.all([
    page1.locator(BOOK_BUTTON_SELECTOR).dispatchEvent('click'),
    page2.locator(BOOK_BUTTON_SELECTOR).dispatchEvent('click')
  ]);
